...
function _formAction(){
    ...
    */
    require_once 'view/aportesocio/Vaporte.php';   
    
}


function _devolverIngresoSocioiAction(){
    $idSocio=$_POST['idSoc'];   

}

function _listarSociosxAnioxIdAction(){


        $idSocio=$_POST['idSoc'];       
        $anioSel = $_POST['anioSel'];
        $mesSoc=$_POST['mesSoc'];       
        $anioSoc = $_POST['anioSoc'];
        
        if($anioSoc<=$anioSel) {

           if($anioSel==$anioSoc) {
                $meses = new Aporte();
                $lmeses=$meses->listarSoloMeses($idSocio,$mesSoc,$anioSoc);
              }
              else{
                $mesSoc=1;
                $meses = new Aporte();
                $lmeses=$meses->listarTodoMeses($idSocio,$mesSoc,$anioSel);
              }
          require 'view/aportesocio/Vlistameses.php';
        }
