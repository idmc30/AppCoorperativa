...
class PDF extends FPDF
{
      ...
      function Header()
      {
          ...
          $this->Cell(80);
          // Título
          $this->Cell(25,10,utf8_decode('LISTADO DE PAGO DE APORTES'),0,0,'C');
          // Arial bold 8
...
/*
  ...
  foreach ($lzona as $dato) {
    ...
    if ( $cont2==1  )
    {
      $pdf->Cell(65,6,utf8_decode("NOMBRE ZONA : ". $dato->nombre_Zon),0,0,'L'); 
      $pdf->Ln(6);
      $pdf->Cell(65,6,utf8_decode("INTEGRADA POR: "),0,0,'L'); 
      $pdf->Ln(6);
      $pdf->Cell(65,6,utf8_decode("   ".$cont2 . ". Distrito: ". $dato->distrito . "   Provincia: ". $dato->provincia . "   Departamento: ". $dato->departamento),0,0,'L');
    }     
    else
    {
      $pdf->Cell(65,6,utf8_decode("   ".$cont2 . ". Distrito: ". $dato->distrito . "   Provincia: ". $dato->provincia . "   Departamento: ". $dato->departamento),0,0,'L');
...
$pdf->Cell(0,10,utf8_decode("Desde: ".$fechaInicio. "  Hasta  " . $fechaFin),0,0,'C');
$pdf->Ln(10);
...
$pdf->SetFillColor(201, 201, 201);
$pdf->Cell(20,10, utf8_decode("Cuota"),1,0,'C',1);
$pdf->SetFillColor(201, 201, 201);
$pdf->Cell(30,10, utf8_decode("Fecha Pago"),1,0,'C',1);
$pdf->SetFillColor(201, 201, 201);
$pdf->Cell(20,10, utf8_decode("Mes"),1,0,'C',1);
$pdf->SetFillColor(201, 201, 201);
$pdf->Cell(26,10,"Estado",1,0,'C',1);
$pdf->SetFillColor(201, 201, 201);
...
//$pdf->Cell(10,10,"",0); //modificar el pading  
  ...
  foreach ($lAportes as $dato) {
      ...
      $pdf->Cell(20,8,utf8_decode($dato->cuota_Apo),1,0,'C');      
      $pdf->Cell(30,8,utf8_decode($dato->fecha_Apo),1,0,'C');
      $pdf->Cell(20,8,utf8_decode($dato->mes_Apo),1,0,'C');
      $pdf->Cell(26,8,utf8_decode($dato->estado_Apo),1,0,'C');
